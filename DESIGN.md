...
### 1.1 목표
`oc-setup`은 OpenCode를 처음 사용하는 개발자가 최적의 초기 환경을 세팅할 수 있게 도와주는 **OpenCode 플러그인 겸 독립 CLI**다.
...
│
├── 플러그인 모드
│   opencode.json의 "plugin" 배열에 "oc-setup" 추가
...
### 1.6 배포
- npm publish (`npm install -g oc-setup` 또는 `npx oc-setup`)
- OpenCode 플러그인 등록 (`opencode.json`의 `"plugin": ["oc-setup"]`)
...
`oc-setup init` 과정에서 사용자에게 인기 플러그인 설치를 제안한다.
...
oc-setup은 스택에 맞는 기본 skill을 생성한다.
...
| `npx oc-setup migrate claude-code` | `setup_migrate` | 마이그레이션 |
...
program.name("oc-setup").description("OpenCode 초기 환경 세팅 도구").version("0.1.0")
